  ...
  export default {
    ...
    methods: {
      ...
      calculateTotal() {
        ...
        const basePrice = this.selectedEvent.base_price;
        let additional = 0;
        ...
        // Aquí validar el código de descuento
      ...
      async validateDiscountCode() {
        ...
        if (this.discountCode) {
          // Suponiendo que hay una API para validar códigos de descuento
          try {
            ...
            if (response.data.valid) {
              discount = response.data.percentage;
            } else {
        ...
        const basePrice = this.selectedEvent.base_price;
        return basePrice * (1 - discount / 100); // Retorna el precio después del descuento
      },
      async purchaseTicket() {
        ...
        // Calcular el valor adicional y total
        let additionalPrice = 0;
        if (this.ticketType === "general") {
          additionalPrice = event.base_price * 0.15; // 15% adicional
        } else if (this.ticketType === "vip") {
          additionalPrice = event.base_price * 0.3; // 30% adicional
        }
  
        const totalPrice = event.base_price + additionalPrice;
